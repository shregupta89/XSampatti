...
const QuickActions = () => {
  ...
  useEffect(() => {
    let uniqueCategories = [...new Set(expenses.map((expense) => expense.category.name))].sort();
    let filtered = uniqueCategories.filter((category) =>
      category?.toLowerCase().includes(searchQuery.toLowerCase())
    );
  ...
  const getCategoryIcon = (categoryName) => {
    ...
    return bestMatch ? categoryMap[bestMatch] : IndianRupee;
  };
  ...
  return (
    <Card className="w-full h-full shadow-sm">
      ...
      {/* Expense Dialog (only one instance outside the map loop) */}
      <ExpenseFormDialog open={open} setOpen={setOpen} category={selectedCategory} />
    </Card>
  );
};
